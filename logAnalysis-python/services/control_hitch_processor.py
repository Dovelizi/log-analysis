# -*- coding: utf-8 -*-
"""
Control Hitch日志专用数据处理器
负责将Control日志数据按照特定规则转换并写入control_hitch_error_mothod表
"""
import json
import re
...
from services.insert_record_service import record_insert_log, LOG_FROM_CONTROL_HITCH


class ControlHitchProcessor:
    """Control Hitch日志数据处理器"""
    ...
    def _init_table(self):
        """初始化control_hitch_error_mothod表"""
        ...
        if self.db_type == 'mysql':
            cursor.execute('''
                ...
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='顺风车错误方法监控表'
            ''')
        else:
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS control_hitch_error_mothod (
    ...
    def _get_field_config(self) -> List[Dict]:
        ...
        # 默认配置 - 适配新表结构
        default_config = [
            {'name': 'method_name', 'source': 'content', 'transform': 'regex:method:([^,]+):1'},
            {'name': 'error_code', 'source': 'content', 'transform': 'regex:code=(\\d+):1|default_code:500'},
            {'name': 'error_message', 'source': 'content', 'transform': 'regex:desc=([^)]+):1|default_if_timeout:system_error'},
    ...
    def _apply_transform(self, value: Any, transform: str, full_data: Dict) -> Any:
        ...
        for t in transforms:
            ...
            if t == 'trim' and isinstance(result, str):
            ...
            elif t.startswith('substr:') or t.startswith('substring:'):
                # 支持 substr:length 或 substr:start:end
                parts = t.split(':')
                if len(parts) >= 2 and isinstance(result, str):
                    if len(parts) == 2:
                        length = int(parts[1])
                        ...
                        end = int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else None
                ...
                if result is None or result == '':
                    fallback_field = t[9:]
                    try:
                        source_field = full_data.get('response_body') or full_data.get('content')
                ...
                if result is None or result == '':
                    result = t[17:]
            elif t.startswith('default_if_timeout:'):
                # 如果当前值为空，检查fullData中是否包含timeout
                if result is None or result == '':
                    default_val = t[19:]
                    content_str = json.dumps(full_data).lower()
                    if 'timeout' in content_str:
                        result = 'timeout'
                    else:
                        result = default_val or 'system_error'
    ...
    def _extract_method_name(self, content: str) -> Optional[str]:
        """
        从content中提取method_name
        格式: method:orderStatusUpdate,failed
    ...
    def _extract_error_code(self, content: str) -> Optional[int]:
        """
        从content中提取error_code
        格式: code=700000
        如果没有code=，则返回默认值500
    ...
    def _extract_error_message(self, content: str) -> Optional[str]:
        """
        从content中提取error_message
        格式: desc=请求过于频繁)
        如果没有desc=，则根据内容判断：
        - 包含timeout则返回'timeout'
        - 否则返回'system_error'
    ...
    def transform_log(self, log_data: Dict, query_transform_config: Dict = None) -> Dict:
        """
        将单条日志数据转换为control_hitch_error_mothod表格式
        ...
        try:
            ...
            result = transform_for_table('control_hitch_error_mothod', log_data, db_config, query_transform_config)
            ...
            if not result.get('error_code'):
                ...
                result['error_code'] = str(error_code) if error_code is not None else None
            ...
            try:
                ...
                if cached_data:
                    ...
                    if existing_id:
                        if self.db_type == 'mysql':
                            update_sql = f'''
                                UPDATE `control_hitch_error_mothod` 
                    ...
                    if self.db_type == 'mysql':
                        check_sql = f'''
                            SELECT id, count, total_count, create_time, update_time FROM `control_hitch_error_mothod` 
                ...
                # 记录入库日志
                record_insert_log(LOG_FROM_CONTROL_HITCH, method_name, content, batch_count)
    ...
    def get_table_data(self, limit: int = 100, offset: int = 0, 
        ...
        if self.db_type == 'mysql':
            cursor.execute(f'''
                SELECT * FROM `control_hitch_error_mothod` 
        ...
        # 获取总数
        if self.db_type == 'mysql':
            cursor.execute('SELECT COUNT(*) as total FROM `control_hitch_error_mothod`')
        else:
            cursor.execute('SELECT COUNT(*) as total FROM control_hitch_error_mothod')
