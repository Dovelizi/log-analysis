# -*- coding: utf-8 -*-
"""
Hitch Supplier Error SP 日志专用数据处理器
负责将日志数据按照特定规则转换并写入hitch_supplier_error_sp表
"""
import json
import re
...
from services.insert_record_service import record_insert_log, LOG_FROM_SUPPLIER_SP


class HitchSupplierErrorSpProcessor:
    """Hitch Supplier Error SP 日志数据处理器"""
        ...
        if self.db_type == 'mysql':
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS hitch_supplier_error_sp (
                    ...
                    content VARCHAR(10240) DEFAULT NULL COMMENT '原始错误上下文或日志详情（用于排查）',
                ...
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='顺风车供应商维度错误明细聚合表（按供应商+方法聚合，当天累计）'
            ''')
        else:
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS hitch_supplier_error_sp (
    ...
    def _get_field_config(self) -> List[Dict]:
        ...
        default_config = [
            ...
            {'name': 'sp_name', 'source': 'sp_name', 'transform': None},
    ...
    def transform_log(self, log_data: Dict, query_transform_config: Dict = None) -> Dict:
        """
        将单条日志数据转换为hitch_supplier_error_sp表格式
        ...
        try:
            ...
            result = transform_for_table('hitch_supplier_error_sp', log_data, db_config, query_transform_config)
            ...
            if not result.get('sp_name'):
                result['sp_name'] = log_data.get('sp_name')
    ...
    def process_logs(self, log_data_list: List[Dict], query_transform_config: Dict = None, query_filter_config: Dict = None) -> Dict:
        ...
        # 表名用于Redis缓存key
        table_name = 'hitch_supplier_error_sp'
            ...
            try:
                ...
                if cached_data:
                    ...
                    if existing_id:
                        if self.db_type == 'mysql':
                            update_sql = f'''
                                UPDATE `hitch_supplier_error_sp` 
                                SET `count` = {ph}, `total_count` = {ph}, `content` = {ph}, `sp_name` = {ph}
                                WHERE id = {ph}
                            '''
                        else:
                            update_sql = f'''
                                UPDATE hitch_supplier_error_sp 
                                SET count = {ph}, total_count = {ph}, content = {ph}, sp_name = {ph}, update_time = CURRENT_TIMESTAMP
                                WHERE id = {ph}
                            '''
                        cursor.execute(update_sql, (batch_count, new_total, content, sp_name, existing_id))
                else:
                    # Redis中没有数据，查询MySQL（查询当天的数据）
                    if self.db_type == 'mysql':
                        check_sql = f'''
                            SELECT id, count, total_count, create_time, update_time FROM `hitch_supplier_error_sp` 
                ...
                # 记录入库日志
                record_insert_log(LOG_FROM_SUPPLIER_SP, method_name, content, batch_count, sp_id)
    ...
    def get_table_data(self, limit: int = 100, offset: int = 0, 
        ...
        allowed_columns = ['id', 'sp_id', 'sp_name', 'method_name', 'content', 'error_code', 'error_message', 
                          'count', 'total_count', 'create_time', 'update_time']
        ...
        # 获取总数
        if self.db_type == 'mysql':
            cursor.execute('SELECT COUNT(*) as total FROM `hitch_supplier_error_sp`')
        else:
            cursor.execute('SELECT COUNT(*) as total FROM hitch_supplier_error_sp')
        ...
        return {
            'columns': ['id', 'sp_id', 'sp_name', 'method_name', 'content', 'error_code', 'error_message',
                       'count', 'total_count', 'create_time', 'update_time'],
            'data': data,
            'total': total,
            'limit': limit,
