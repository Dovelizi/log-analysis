...
public class ClsQueryService {
     ...
     * 单独抽出，便于后续被其它地方复用/替换。
     *
     * P2b-3 起：使用 MyBatis-Plus {@link com.loganalysis.topic.infrastructure.persistence.mapper.TopicMapper}。
     */
    @Service
    public static class TopicLookup {
        @Autowired
        private com.loganalysis.topic.infrastructure.persistence.mapper.TopicMapper topicMapper;

        public String regionOfTopic(String topicId) {
            if (topicId == null || topicId.isEmpty()) return null;
            try {
                com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<
                        com.loganalysis.topic.infrastructure.persistence.po.TopicPO> w =
                        new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>();
                w.eq(com.loganalysis.topic.infrastructure.persistence.po.TopicPO::getTopicId, topicId)
                 .select(com.loganalysis.topic.infrastructure.persistence.po.TopicPO::getRegion)
                 .last("LIMIT 1");
                List<com.loganalysis.topic.infrastructure.persistence.po.TopicPO> rs =
                        topicMapper.selectList(w);
                return rs.isEmpty() ? null : rs.get(0).getRegion();
            } catch (Exception e) {
                return null;
            }
        }
