...
import com.loganalysis.queryconfig.application.QueryConfigService;
...
public class SearchLogsController {
    ...
    public ResponseEntity<?> statistics() {
        Map<String, Object> ret = new LinkedHashMap<>();
        Long totalLogs = jdbc.queryForObject("SELECT COUNT(*) FROM log_records", Long.class);
        Long totalConfigs = jdbc.queryForObject("SELECT COUNT(*) FROM query_configs", Long.class);
        Long totalTopics = jdbc.queryForObject("SELECT COUNT(*) FROM log_topics", Long.class);
        ret.put("total_logs", totalLogs == null ? 0L : totalLogs);
        ret.put("total_configs", totalConfigs == null ? 0L : totalConfigs);
        ret.put("total_topics", totalTopics == null ? 0L : totalTopics);
        ret.put("time_distribution", jdbc.queryForList(
        ...
        ret.put("topic_distribution", jdbc.queryForList(
