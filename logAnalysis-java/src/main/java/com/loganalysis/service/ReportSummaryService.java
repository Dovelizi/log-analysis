...
public class ReportSummaryService {
    ...
    /** 获取日报汇总（对齐 Python get_report_summary） */
    public Map<String, Object> summary(String date) {
        ...
        DateRange dr = new DateRange(targetDate, targetDate,
                targetDate + " 00:00:00", targetDate + " 23:59:59");

        // Control Hitch
        try {
            Map<String, Object> control = dashboard.hitchStatistics(
        ...
        // GW Hitch
        try {
            Map<String, Object> gw = dashboard.hitchStatistics(
        ...
        // Cost Time
        try {
            Map<String, Object> ct = dashboard.costTimeStatistics(dr, 1, 10, 1, 10);
        ...
        // Supplier SP
        try {
            Map<String, Object> sp = dashboard.supplierSpStatistics(dr, 1, 10);
    ...
    public Map<String, Object> weeklyNewErrors(String endDate) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            String curStart = cur + " 00:00:00", curEnd = cur + " 23:59:59";
            String prevStart = prev + " 00:00:00", prevEnd = prev + " 23:59:59";

            // 当天组合（含 total_count）
            List<Map<String, Object>> curRows = jdbc.queryForList(
                    "SELECT method_name, error_code, error_message, SUM(total_count) as total_count " +
            ...
            // 前一天 key 集合
            List<Map<String, Object>> prevRows = jdbc.queryForList(
                    "SELECT DISTINCT method_name, error_code " +
                    "FROM control_hitch_error_mothod " +
                    "WHERE create_time BETWEEN ? AND ?",
                    prevStart, prevEnd);
            Set<String> prevKeys = new HashSet<>(prevRows.size());
            for (Map<String, Object> r : prevRows) {
                prevKeys.add(keyOf(r.get("method_name"), r.get("error_code")));
            }

            List<Map<String, Object>> newErrors = new ArrayList<>();
            for (Map<String, Object> r : curRows) {
                String k = keyOf(r.get("method_name"), r.get("error_code"));
                if (!prevKeys.contains(k)) newErrors.add(r);
            }
