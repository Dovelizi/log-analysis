...
public class HitchSupplierErrorTotalProcessor {
            ...
            try {
                ...
                if (cached != null) {
                    ...
                    if (existingId instanceof Number) {
                        jdbc.update(
                                "UPDATE `hitch_supplier_error_total` SET `count` = ?, `total_count` = ?, `content` = ? WHERE id = ?",
                    ...
                    if (existing != null) {
                        ...
                        jdbc.update(
                                ...
                                "(`sp_id`, `method_name`, `content`, `error_code`, `error_message`, `count`, `total_count`) " +
                                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                                spId, methodName, content, errorCode, errorMessage,
    ...
    public Map<String, Object> getTableData(int limit, int offset, String orderBy, String orderDir) {
        ...
        List<Map<String, Object>> rows = jdbc.queryForList(
                "SELECT * FROM `hitch_supplier_error_total` ORDER BY `" + safeOrderBy + "` " + dir +
                " LIMIT ? OFFSET ?", limit, offset);
        Long total = jdbc.queryForObject("SELECT COUNT(*) FROM `hitch_supplier_error_total`", Long.class);
        Map<String, Object> ret = new LinkedHashMap<>();
